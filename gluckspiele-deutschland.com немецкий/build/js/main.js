...
if(headerBottom && hero) {
  
  const headerBottomStyles = headerBottom.getBoundingClientRect();
  
  const headerBottomHeight = headerBottomStyles.height;
 
  hero.style.marginTop = -`${headerBottomHeight / 2}` + "px";
}

const unterkunftSlider = document.querySelector(".unterkunft-slider");

if(unterkunftSlider) {
  const unterkunftSwiper = new Swiper(unterkunftSlider, { 
    observer: true,
    observeParents: true,
    watchOverflow: true,
    slidesPerView: "auto",
    ...
    pagination: {
      el: '.unterkunft-slider-pagination',
      clickable: true,
    },
    navigation: {
      nextEl: '.unterkunft-slider-next',
      prevEl: '.unterkunft-slider-prev',
    },
  });
} 
// };